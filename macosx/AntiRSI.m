...
- (void)installTimer:(double)interval
{
    ...
    mtimer = [NSTimer scheduledTimerWithTimeInterval:interval target:self selector:@selector(tick:) userInfo:nil repeats:YES];
}

- (void)setSample_interval:(NSString *)string
{
    sample_interval = 1;
    if ([string isEqualToString:@"Super Smooth"]) sample_interval = 0.1;
    if ([string isEqualToString:@"Smooth"]) sample_interval = 0.33;
    if ([string isEqualToString:@"Normal"]) sample_interval = 1;
    if ([string isEqualToString:@"Low"]) sample_interval = 2;
...
- (void)awakeFromNib
{
    ...
    // initial colors
    elapsed = [NSColor colorWithCalibratedRed:0.77 green:0 blue:0 alpha:0.9];
    taking = [NSColor colorWithCalibratedRed:0.04 green:0.46 blue:0.04 alpha:0.9];
    ...
    main_window = [[NSWindow alloc] initWithContentRect:[view frame] styleMask:NSBorderlessWindowMask backing:NSBackingStoreBuffered defer:YES];
    ...
    [self bind:@"micro_pause_period" toObject:userDefaultsController withKeyPath:@"values.micro_pause_period" options:nil];
    [self bind:@"micro_pause_duration" toObject:userDefaultsController withKeyPath:@"values.micro_pause_duration" options:nil];
    [self bind:@"work_break_period" toObject:userDefaultsController withKeyPath:@"values.work_break_period" options:nil];
    [self bind:@"work_break_duration" toObject:userDefaultsController withKeyPath:@"values.work_break_duration" options:nil];
    [self bind:@"sample_interval" toObject:userDefaultsController withKeyPath:@"values.sample_interval" options:nil];
    [self bind:@"draw_dock_image" toObject:userDefaultsController withKeyPath:@"values.draw_dock_image" options:nil];
    [self bind:@"lock_focus" toObject:userDefaultsController withKeyPath:@"values.lock_focus" options:nil];
    ...
    [self bind:@"elapsed" toObject:userDefaultsController withKeyPath:@"values.elapsed" options:unarchive];
    [self bind:@"taking" toObject:userDefaultsController withKeyPath:@"values.taking" options:unarchive];
    [self bind:@"background" toObject:userDefaultsController withKeyPath:@"values.background" options:unarchive];
    ...
    // about dialog
    sVersion = [NSString stringWithFormat:@"%@", [[[NSBundle mainBundle] infoDictionary] objectForKey:@"CFBundleVersion"]];
...
- (void)drawNextBreak:(int)seconds {
    ...
    // nice hours, minutes ...
    if (minutes > 60) {
        [next_break setStringValue:[NSString stringWithFormat:@"Next break in %d:%02d hours", minutes / 60, minutes % 60]];
    } else {
